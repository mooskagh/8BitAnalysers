...
void DrawMemoryDiffUI(FCpcEmu* pCpcEmu)
{
	FCodeAnalysisViewState& viewState = pCpcEmu->CodeAnalysis.GetFocussedViewState();
	...
	if(ImGui::Button("SnapShot"))
	{
		...
		{
			g_DiffSnapShotMemory[addr] = pCpcEmu->ReadByte( addr);
		}
	...
	if (g_bSnapshotAvailable)
	{
		...
		if (ImGui::Button("Diff"))
		{
			...
			for (int addr = startAddr; addr < (1 << 16); addr++)
			{
				if (pCpcEmu->ReadByte( addr) != g_DiffSnapShotMemory[addr])
	...
	if(ImGui::BeginChild("DiffedMemory", ImVec2(0, 0), true, window_flags))
	{
		for(const uint16_t changedAddr : g_DiffChangedLocations)
		{
			...
			ImGui::SameLine();
			ImGui::Text("%s\t%s\t%s", NumStr(changedAddr), NumStr(g_DiffSnapShotMemory[changedAddr]), NumStr(pCpcEmu->ReadByte( changedAddr)));
			ImGui::SameLine();
